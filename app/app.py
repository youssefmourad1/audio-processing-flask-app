...
# Import SpeechLabelEntity at the top level
from nemo.collections.common.parts.preprocessing.collections import SpeechLabelEntity
...
# Global models dictionary to hold all preloaded models
models = {}

# Explicitly include the API keys
GROQ_KEY = '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'
HG_KEY = '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'

def initialize_models():
    global models
    # Initialize Demucs model
    models['demucs'] = get_model('htdemucs')
    models['demucs'].eval()
    # Initialize SpeechBrain embedding model
    models['embedding_model_speechbrain'] = SpeakerRecognition.from_hparams(
    ...
    models['diarizer_config'] = config

    # Initialize Groq client
    models['groq_client'] = Groq(api_key=GROQ_KEY)

def process_audio(audio_path, output_dir, brands_file, reference_audio_path, metrics_csv_path):
    ...
    # Step 2: Create manifest for diarization task
    step_start_time = time.time()
    ...
    step_start_time = time.time()
    rttm_file, adjusted_segments, waveform, sample_rate, adjusted_segments2 = run_diarization(
        mono_audio_path, manifest_path, output_dir
    )
    ...
    if len(adjusted_segments2) < 4:
        ...
        # Step 5: Load special words from file
        step_start_time = time.time()
        ...
        # Step 6: Process speaker embeddings and identify best match
        step_start_time = time.time()
        best_match, filtered_segments = process_speaker_embeddings(
            adjusted_segments, special_words, full_audio, reference_audio_path
        )
        metrics['process_speaker_embeddings'] = time.time() - step_start_time

        # Step 7: Transcribe and correct the full conversation
        step_start_time = time.time()
        corrected_full_conversation = transcribe_and_correct_conversation(
            filtered_segments, full_audio, special_words
        )
        metrics['transcribe_and_correct_conversation'] = time.time() - step_start_time

        # Build the transcription output path
        ...
        with open(transcription_output_path, "w", encoding="utf-8") as f:
            speaker_lines = [
                ...
                for line in corrected_full_conversation.split("\n")
...
def apply_demucs_and_deepfilter(audio_path):
    ...
    model = models['demucs']  # Use preloaded model
...
def apply_deepfilter(audio_path):
    ...
    output_dir = "/tmp"  # Ensure this is a writable directory
    output_file = filename.replace(".wav", "_DeepFilterNet3.wav")
    output_path = os.path.join(output_dir, output_file)
    logger.info(f"Applying DeepFilterNet to {filename}...")

    # Ensure output directory exists
    os.makedirs(output_dir, exist_ok=True)

    # Run DeepFilterNet with input and output specified
    exit_code = os.system(f'deepFilter -i "{audio_path}" -o "{output_path}"')
    if exit_code != 0 or not os.path.exists(output_path):
        raise RuntimeError(f"DeepFilterNet failed to process the file {audio_path}")
    logger.info(f"DeepFilterNet applied. Output saved as {output_file}")
    return output_path

def convert_to_mono(audio_path, output_dir):
    waveform, sample_rate = torchaudio.load(audio_path)
    if waveform.shape[0] > 1:  # Check if audio is multi-channel
...
def create_manifest(mono_audio_path, output_dir):
    rttm_path = os.path.join(
        output_dir, "pred_rttms", f"{os.path.basename(mono_audio_path).replace('.wav', '.rttm')}"
    )
    meta = {
...
def run_diarization(mono_audio_path, manifest_path, output_dir):
    ...
    # Set num_workers to 0 to prevent multiprocessing
    config.diarizer.vad.parameters.num_workers = 0
    config.diarizer.speaker_embeddings.parameters.num_workers = 0
    config.diarizer.diarizer_model.parameters.num_workers = 0  # Ensure all num_workers are set to 0
    ...
    rttm_file = os.path.join(
        output_dir, 'pred_rttms', os.path.basename(mono_audio_path).replace('.wav', '.rttm')
    )
    waveform, sample_rate = torchaudio.load(mono_audio_path)

    # Parse the RTTM file and adjust segments
...
def adjust_segments(rttm_file, waveform, sample_rate):
    ...
    with open(rttm_file, 'r') as f:
        ...
        for line in f:
            ...
            if speaker == current_speaker:
                ...
                if current_speaker is not None:
                    # Add to adjusted_segments2 without splitting
                    adjusted_segments2.append((current_speaker, current_start_time, current_end_time, current_waveform))
                    # Add to adjusted_segments after splitting
                    adjusted_segments.extend(
                        split_long_segments(current_speaker, current_start_time, current_end_time, current_waveform, sample_rate)
                    )
        ...
        if current_speaker is not None:
            # Add the last segment to adjusted_segments2 without splitting
            adjusted_segments2.append((current_speaker, current_start_time, current_end_time, current_waveform))
            # Add the last segment to adjusted_segments after splitting
            adjusted_segments.extend(
                split_long_segments(current_speaker, current_start_time, current_end_time, current_waveform, sample_rate)
            )
...
def process_speaker_embeddings(adjusted_segments, special_words, full_audio, reference_audio_path):
    ...
    embedding_model_speechbrain = models['embedding_model_speechbrain']

    # Load reference audio and get embedding
    ...
    # Initialize dictionaries for similarities and speaker audios
    similarities = {}
    speaker_audios = {}
    ...
    for segment in adjusted_segments:
        ...
        # Add this segment to the combined audio for the speaker
    ...
    for speaker, audio in speaker_audios.items():
        ...
        # Get the embedding for the speaker's audio
        speaker_embedding = get_audio_embedding_speechbrain(speaker_audio, embedding_model_speechbrain)

        # Compute cosine similarity between reference and speaker embeddings
    ...
    logger.info(f"The speaker matching the reference is: {best_match}")

    # Filter segments to keep only those of the best speaker
...
def get_audio_embedding_speechbrain(audio_segment, embedding_model, sample_rate=16000, min_length=16000):
    ...
    samples = samples.mean(axis=0, keepdim=True)  # Ensure mono
...
def transcribe_and_correct_conversation(filtered_segments, full_audio, special_words):
    ...
    for segment in filtered_segments:
        transcription = transcribe_segment(
            segment['start_time'], segment['end_time'], segment['speaker'], full_audio
        )
        if transcription and transcription.strip():
            full_conversation_text += f"Speaker {segment['speaker']}: {transcription}\n"
        else:
            logger.info(f"Transcription for speaker {segment['speaker']} is empty. Skipped.")
